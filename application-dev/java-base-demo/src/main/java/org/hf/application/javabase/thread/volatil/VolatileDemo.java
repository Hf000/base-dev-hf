 ...
 *  有序性, 指的是你写的代码的顺序要和最终执行的指令保持一致。因为在Java内存模型中，允许编译器和处理器对指令进行重排序，重排序过程不会影响到单线程程序的执行，却会影响到多线程并发执行的正确性。
 * 对变量加上volatile关键字,当对此变量进行写操作时,JVM会向处理器发送一条lock前缀指令,将缓存中的变量回写到主存中去
 *
 * @author HF
 * @date 2022-10-31
 **/
