...
public class OrderDaoImpl implements OrderDao {
    ...
    public void add(Order order) {
        jdbcTemplate.update("INSERT INTO order_info(id,money,status,item_id,num,username,paymoney,couponsId) values(?,?,?,?,?,?,?,?)", order.getId(), order.getMoney(), order.getStatus(), order.getItemId(), order.getNum(), order.getUsername(), order.getPayMoney(), order.getCouponsId());
    }
