...
public class RsaUtil {
     ...
     * @param data 待加密数据
     */
    private static String doPublicEncrypt(String data, Key publicKey, Integer keySize) throws GeneralSecurityException, IOException {
        int maxEncryptBlock;
        if (keySize != null) {
            maxEncryptBlock = keySize / 8 - 11;
        } else {
            maxEncryptBlock = ((RSAKey) publicKey).getModulus().bitLength() / 8 - 11;
        }
     ...
     * @param data 待解密数据
     */
    private static String doPrivateDecrypt(String data, Key privateKey, Integer keySize) throws GeneralSecurityException, IOException {
        int maxDecryptBlock;
        if (keySize != null) {
            maxDecryptBlock = keySize / 8;
        } else {
            maxDecryptBlock = ((RSAKey) privateKey).getModulus().bitLength() / 8;
        }
        Cipher cipher = Cipher.getInstance(FULL_CIPHER_NAME);
