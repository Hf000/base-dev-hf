...
public class RedisUtilComponent {
    ...
    public RedisAtomicLong getRedisCounter(String redisKey, Long initialValue) {
        ...
        RedisTemplate<String, String> redisTemplate = SpringContextUtil.getBean(StringRedisTemplate.class);
     ...
     * 向set添加一个元素
     * @param key       缓存key
     * @param element   缓存集合元素
     */
    public Long sadd(String key, String element) {
        return redisTemplate.opsForSet().add(key, element);
    }
    ...
    public Long srem(String key, String element) {
        return redisTemplate.opsForSet().remove(key, element);
    }

    public static String generateKey(String className, String methodName, Object... params) {
        String redisKey = className + ":" + methodName + ":"
                + objArrToDeepStrEncode(params, "->");
        log.debug("RedisUtilComponent.generateKey generated key={}", redisKey);
        return redisKey;
    }

    public static String objArrToDeepStrEncode(@Nullable Object[] arr, String delim) {
        try {
            return AesUtil.encryptDefault(arrayToDelimitedDeepString(arr, delim));
        } catch (Exception e) {
            throw new BusinessException(e);
        }
    }

    public static String arrayToDelimitedDeepString(@Nullable Object[] arr, String delim) {
        if (ObjectUtils.isEmpty(arr)) {
            return "";
        }
        if (arr.length == 1) {
            return JSON.toJSONString(arr[0]);
        }

        StringJoiner sj = new StringJoiner(delim);
        for (Object elem : arr) {
            sj.add(JSON.toJSONString(elem));
        }
        return sj.toString();
    }
    ...
    public Long hdel(String key, String hashKey) {
        return redisTemplate.<String, String>opsForHash().delete(key, hashKey);
    }

    /**
     * <div style="color:blue; font-weight: bold;">一般业务逻辑不推荐使用</div>
     * 执行lua脚本，例如：
     ...
     */
    @Deprecated
    public <T> T luaScript(String script, Class<T> resultType, List<String> keys, Object... argvs) {
        RedisScript<T> redisScript = RedisScript.of(script, resultType);
        return redisTemplate.execute(redisScript, keys, argvs);
    }
}